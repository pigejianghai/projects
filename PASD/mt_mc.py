...
from unet_transfer import UNetResNet
from unetcls_mc import UNet, CombinedLoss
...
def training(train_loader, len_train, net, criterion, epoch, writer, 
    ...
    for images, masks, labels in train_loader:
        ...
        outputs, outputs_msk = net(images)
        # if fold // 10 == 0:
        loss = criterion(outputs, labels, outputs_msk, masks)
        # else: 
...
def validating(val_loader, len_val, net, criterion, epoch, writer, 
               device, 
               best_auc, 
               fold, num_classes, checkpoint_name):#
    net.eval()
    val_loss = 0.0
    best_auc = best_auc
    # best_acc = best_acc
    ...
    with torch.no_grad():
        for images, masks, labels in val_loader:
            ...
            # else:
            #     loss = loss_cls(outputs, labels)
            output_hat = F.softmax(outputs, dim=1)
    ...
    return best_auc, total_auc.item()#epoch_loss, , best_acc
...
if __name__ == '__main__':
    ...
    checkpoint_fold = ROOT_PATH + os.sep + 'checkpoint' + os.sep + DATE_NET + '_URes_alpha0_5'
    ...
    CRITERION_MT = CombinedLoss(loss1=loss_cls, loss2=loss_seg, alpha=0.5)
    NUM_SPLITS = 5
    EPOCHS = 500
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(path_list, label_list)):
        ...
        MODEL = UNetResNet(num_classes=10, num_filters=16)#weights=models.ResNet18_Weights
    ...
    ############################################
        ...
        for epoch in range(EPOCHS):
            ...
            training(train_loader=train_loader, len_train=len(train_data), 
                     net=MODEL, criterion=CRITERION_MT, 
            ...
            #######################################
            #######################################val_epoch_loss, 
            best_auc, val_epoch_auc = validating(val_loader=val_loader, len_val=len(val_data), 
                                                 net=MODEL, criterion=CRITERION_MT, 
                                                 epoch=epoch, writer=writer, 
                                                 device=DEVICE, 
